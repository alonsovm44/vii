...
table_set(env, "i", runtime_binop(41, runtime_var_get(env, "i"), val_num(1)));
  ...
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("if (argc > 1 && strcmp(argv[1], \"--version\") == 0) {")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("  printf(\"vii 2.0.0 bootstrapped\\n\");")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("  return 0;")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("}")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("if (argc > 1 && strcmp(argv[1], \"--help\") == 0) {")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("  printf(\"vii - a minimalist programming language\\n\");")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("  printf(\"Usage: vii <file.vii> [args...]\\n\");")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("  return 0;")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("}")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("if (argc < 2) {")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("  fprintf(stderr, \"Error: no input file\\n\");")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("  return 1;")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("}")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("FILE* f = fopen(argv[1], \"rb\");")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("if (!f) { fprintf(stderr, \"Error: cannot open '%s'\\n\", argv[1]); return 1; }")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("fseek(f, 0, SEEK_END);")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("long size = ftell(f);")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("fseek(f, 0, SEEK_SET);")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("char* source = malloc(size + 1);")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("fread(source, 1, size, f);")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("source[size] = 0;")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("fclose(f);")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("Value* arg = val_list();")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("for (int i = 2; i < argc; i++) {")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("  if (arg->item_count >= arg->item_cap) val_list_grow(arg);")});
  ...
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("// TODO: lex, parse, and evaluate the source code")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("printf(\"Read file: %s\\n\", argv[1]);")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("printf(\"Size: %ld bytes\\n\", size);")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("free(source);")});
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("return 0;")});
  table_set(env, "indent_lvl", runtime_binop(42, runtime_var_get(env, "indent_lvl"), val_num(1)));
  runtime_call(table_get(env, "emit"), env, 1, (Value*[]){val_str("}")});
