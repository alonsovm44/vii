...
static void emit_c_header(FILE *f) {
    ...
    fprintf(f, "static Value* runtime_str_concat(const char* a, const char* b) { size_t len_a = strlen(a), len_b = strlen(b); char* buf = malloc(len_a + len_b + 1); if (!buf) return val_str(\"\"); memcpy(buf, a, len_a); memcpy(buf + len_a, b, len_b + 1); Value* r = val_str(buf); free(buf); return r; }\n");
